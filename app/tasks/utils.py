...
async def delete_inconsistent_db_data(
    ...
    for db_menu in db_data:
        ...
        if table_menu is None:
            menu_obj: Any
            menu_obj = await menu_crud.get(db_menu['id'], session)
            await menu_crud.remove(menu_obj, session)
        ...
        for db_submenu in db_menu['submenus']:
            ...
            if table_submenu is None:
                submenu_obj: Any
                submenu_obj = await submenu_crud.get(db_submenu['id'], session)
                await submenu_crud.remove(submenu_obj, session)
            ...
            for db_dish in db_submenu['dishes']:
                ...
                if table_dish is None:
                    dish_obj: Any
                    dish_obj = await dish_crud.get(db_dish['id'], session)
                    await dish_crud.remove(dish_obj, session)
    ...
    for table_menu in table_data:
        ...
        for menu in db_data:
            if menu['title'] == table_menu.title:
                ...
                break
        if db_menu is None:
            db_menu_obj = await menu_crud.create(
                MenuCreate(
...
async def _update_db_submenus(
    ...
    for table_submenu in table_menu.submenus:
        ...
        if db_submenu is None:
            db_submenu_obj = await submenu_crud.create(
                SubmenuCreate(
        ...
        if submenu_created is False:
            for dish in db_submenu['dishes']:
                if dish['title'] == table_dish.title:
                    ...
                    break
        if db_dish is None:
            db_dish_obj = await dish_crud.create(
                DishCreate(
